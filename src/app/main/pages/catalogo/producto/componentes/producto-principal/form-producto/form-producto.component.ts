...
export class FormProductoComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.productoEditar) {
      this.formProducto = this.formBuilder.group({
        //codModulo: new FormControl(this.productoEditar?.codModulo),
        codigoProducto: new FormControl(this.productoEditar?.codigoProducto),
        codigoBarras: new FormControl(this.productoEditar?.codigoBarras),
        descripcion: new FormControl(this.productoEditar?.descripcion, Validators.required),
        grupo: new FormControl(this.productoEditar?.grupo),
        medida: new FormControl(this.productoEditar?.medida),
        precioCosto: new FormControl(this.productoEditar?.precioCosto, Validators.required),
        //precioCompra: new FormControl(this.productoEditar?.precioCompra),
      ...
      this.formProducto = this.formBuilder.group({
        //codModulo: new FormControl(this.codModulo),
  ...
  addRegistro() {
    if (this.formProducto?.valid) {
      ...
      this.producto = new Producto({
        codigo: 0,
        codModulo: 2, //productoTemp.codModulo,
        codigoProducto: productoTemp.codigoProducto,
        codigoBarras: productoTemp.codigoBarras,
        descripcion: productoTemp.descripcion,
        grupo: productoTemp.grupo,
        medida: productoTemp.medida,
    ...
    */
    if (this.productoEditar) {
      ...
      this.productoService.guardarProducto(this.producto['data']).subscribe({
        next: (response) => {
          if (this.descripcionChild == undefined || this.descripcionChild != null || this.descripcionChild != '') {
  ...
  get codigoProductoField() {
    return this.formProducto.get('codigoProducto');
  }
  get codigoBarrasField() {
    return this.formProducto.get('codigoBarras');
  }
  get grupoField() {
    return this.formProducto.get('grupo');
  }
  get medidaField() {
    return this.formProducto.get('medida');
  }
